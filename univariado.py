# -*- coding: utf-8 -*-
"""
Created on Tue Oct 19 18:47:45 2021
...
def corr_pearson(df):
    ...
    plt.xticks(range(df.select_dtypes(['number']).shape[1]), df.columns, fontsize=8, rotation=45)
    plt.yticks(range(df.select_dtypes(['number']).shape[1]), df.columns, fontsize=8)
    cb = plt.colorbar()
...
def Corr_Spearman(df):
    corr_sper = pd.DataFrame(sta.spearmanr(df).correlation, columns= df.columns, index=df.columns)
    f = plt.figure(figsize=(9, 11))
